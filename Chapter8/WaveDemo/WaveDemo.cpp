...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE prevInstance, PSTR cmdLine, int showCmd)
{
	...
	Wave theApp(hInstance);
	if (!theApp.Init())
		return 0;
	return theApp.Run();
}

Wave::Wave(HINSTANCE hInstance)
	:D3DApp(hInstance), pHillVB(0), pHillIB(0),pWavesVB(0),pWavesIB(0),
	mGridIndexCount(0),pGrassMapSRV(0),pWaveMapSRV(0),mWaveTexOffset(0.0f,0.0f),
	mEyePosW(0.0f,0.0f,0.0f),mThea(1.3f*MathHelper::Pi), mPhi(0.4f*MathHelper::Pi), mRadius(80.0f)
{
mMainWndCaption = L"Wave Demo";
...
void Wave::UpdateScene(float dt)//update the view matrix ; the camera position
{
...
void Wave::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void Wave::OnMouseUp(WPARAM	btnState, int x, int y)
{
	ReleaseCapture();
}

void Wave::OnMouseMove(WPARAM btnState, int x, int y)
{
...
XMFLOAT3 Wave::GetNormal(float x, float z) const
{
	//n=(-df/dx,1,-df/dz)
...
void Wave::CreateHillGeometryBuffers()
{
...
void Wave::CreateWaveGeometryBuffers()
{
